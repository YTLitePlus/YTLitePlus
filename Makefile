export TARGET = iphone:clang:18.5:14.0
...
export ADDITIONAL_CFLAGS = -I$(THEOS_PROJECT_DIR)/Tweaks/RemoteLog -I$(THEOS_PROJECT_DIR)/Tweaks -fmodules-validate-once-per-build-session # Allow YouTubeHeader to be accessible using #include <...>
...
YTLitePlus_CFLAGS = -fobjc-arc -Wno-deprecated-declarations -Wno-unsupported-availability-guard -Wno-unused-but-set-variable -fmodules-validate-once-per-build-session -DTWEAK_VERSION=$(PACKAGE_VERSION) $(EXTRA_CFLAGS)
